...
async def budget(interaction: discord.InterInteraction if False else discord.Interaction):
    conn = connect()
    cur = conn.cursor()
...
async def close_auction(channel, auction_id: int):
    ...
    if auction["highest_bidder_id"]:
        ...
        if manager and manager["budget"] >= auction["highest_bid"]:
            ...
            cur.execute("""
                UPDATE players SET owner_discord_id = ?, sold_price = ?
                WHERE id = ?
...
async def offri(interaction: discord.Interaction, prezzo: int):
    ...
    if prezzo < auction["highest_bid"] + MIN_RAISE:
        ...
        await interaction.response.send_message(
            f"Devi offrire almeno {auction['highest_bid'] + MIN_RAISE}.",
            ephemeral=True
        )
        return
